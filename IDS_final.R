...
# ------------------------------------------------------------
# 8) Summary tables (Tables 1–6)
# ------------------------------------------------------------

# ---- Table 1: Variable Descriptions ----
table_variables <- tibble::tribble(
  ~Name, ~Type, ~Description,
  "id", "character", "Unique song identifier across Billboard dataset.",
  "song", "character", "Song title (lowercased and trimmed).",
  "band_singer", "character", "Primary performer, band, or singer name.",
  "year", "integer", "Year when the song appeared on Billboard chart.",
  "danceability", "numeric", "Suitability for dancing (0.0–1.0). Combines tempo, rhythm stability, and beat strength.",
  "energy", "numeric", "Perceived intensity and activity of a track (0.0–1.0).",
  "loudness", "numeric", "Overall loudness of the track in decibels (–60 dB to 0 dB).",
  "speechiness", "numeric", "Presence of spoken words (0.0–1.0). Higher values indicate more speech-like recordings.",
  "acousticness", "numeric", "Confidence measure of whether the track is acoustic (0.0–1.0).",
  "instrumentalness", "numeric", "Predicts whether a track contains no vocals. Closer to 1 = more instrumental.",
  "liveness", "numeric", "Probability the recording is live (0.0–1.0).",
  "valence", "numeric", "Musical positiveness or cheerfulness (0.0–1.0).",
  "tempo", "numeric", "Estimated tempo in beats per minute (BPM).",
  "duration_ms", "integer", "Track duration in milliseconds.",
  "hit_group", "factor", "Binary factor indicating Top10 vs NotTop10 chart membership."
)

readr::write_csv(table_variables, "output_variable_description.csv")
message("Table 1 (Variable Descriptions) saved as output_variable_description.csv")

# ---- Table 2: Clean Descriptive Statistics (one row per variable) ----
...
message("Table 2 (Descriptive Statistics) saved as output_table_descriptive_stats_clean.csv")

# ---- Table 3 – dataset sizes (pipeline overview) ----
tbl_sizes <- tibble(
  dataset = c(
    "Billboard raw",
    "Billboard cleaned",
...
# ---- Table 4 – Top10 vs NotTop10 counts ----
tbl_hits <- full_data %>%
  count(hit_group, name = "n_songs")

# ---- Table 5 – overall mean audio features ----
tbl_audio_stats <- full_data %>%
  summarise(
    n                 = n(),
...
# ---- Table 6 – Top10 share by year ----
table_top10_year <- full_data %>%
  ...
  ungroup() %>%
  filter(hit_group == "Top10") %>%
  select(year, top10_count = n, total_songs, top10_share = prop)

# ---- Table 7 – mean audio features by success group ----
table_audio_by_hit <- full_data %>%
  group_by(hit_group) %>%
  summarise(
    n_songs      = n(),
    danceability = mean(danceability, na.rm = TRUE),
...
# ---- Table 8 – feature differences (Top10 − NotTop10) ----
table_feature_diff <- full_data %>%
  select(
    hit_group, danceability, energy, tempo, loudness,
  ...
  group_by(hit_group, feature) %>%
  summarise(
    mean_value = mean(value, na.rm = TRUE),
    .groups    = "drop"
  ) %>%
  pivot_wider(
    names_from  = hit_group,
    values_from = mean_value
  ) %>%
  mutate(diff_top10 = Top10 - NotTop10) %>%
  arrange(desc(diff_top10))

# Save all tables to CSV for the report
write_csv(tbl_sizes,          "output_table_pipeline.csv")
write_csv(tbl_hits,           "output_table_success.csv")
write_csv(tbl_audio_stats,    "output_table_audio_summary.csv")
write_csv(table_top10_year,   "output_table_top10_by_year.csv")
write_csv(table_audio_by_hit, "output_table_audio_by_success.csv")
write_csv(table_feature_diff, "output_table_feature_differences.csv")
